...
for producto in productos:
    ...
    for sucursal in sucursales_producto.keys():
        if sucursal == 4:
            for leadtime in lista_leadtime:
                print(f'\n-- PRODUCTO: {producto} SUCURSAL: {sucursal} ---\n')
                demanda_sucursal = demanda_producto[sucursal]

                folder = f'resultados/lead_time_{leadtime}_t_revision_{t_revision}/{producto}'
                ...
                for j in range(0, len(valores_politica)):
                    ...
                    for i in range(0, replicas):
                        if politica == 'pronostico':
                            shutil.copy(
                                f"pronostico_demanda/excel_branches/branch{sucursal}.xlsx", f"pronostico_demanda/excel_branches/branch{sucursal}(5).xlsx"
                            )
